...
<div class="container mx-auto px-4 py-6">
    <div class="bg-white p-6 rounded shadow mb-6">
        <div class="flex items-center mb-6">
            ...
            <div class="">
                <span class="">{{ $post->user->first_name.' '.$post->user->last_name }}</span>
        ...
        <div class="mt-6 flex items-center space-x-4">
            <form method="POST" action="{{ route('post.like', $post) }}">
                @csrf
                <button type="submit" class=" flex items-center text-black py-2 px-4 rounded gap-2">
    ...
    <div class="bg-white p-6 rounded shadow">
                ...
                <div class="flex items-start gap-4">
                    ...
                    <div class="flex-1">
                        ...
                        <form id="reply-form-{{ $comment->id }}" method="POST" action="{{ route('comments.store', $post) }}" class="hidden mt-2">
                            ...
                            <textarea name="body" class="w-full border rounded p-2 mt-2" placeholder="Balas komentar..."></textarea>
                            <button type="submit" class="mt-2 bg-blue-500 hover:bg-blue-600 text-white py-1 px-4 rounded">Kirim Balasan</button>
                        </form>
        ...
        <form method="POST" action="{{ route('comments.store', $post) }}" class="mt-6">
            @csrf
            <textarea name="body" class="w-full border rounded p-4" placeholder="Tulis komentar baru..."></textarea>
            <button type="submit" class="mt-4 bg-green-500 hover:bg-green-600 text-white py-2 px-4 rounded cursor-pointer">Komentar</button>
        </form>
    </div>
</div>
