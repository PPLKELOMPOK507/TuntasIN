        ...
        <div class="d-flex flex-wrap gap-2 justify-content-between align-items-center mt-4" style="flex-direction: column; align-items: stretch;">
            ...
            @if($refund->status === 'pending' && empty($refund->provider_response))
                <div class="d-flex gap-2 w-100" style="justify-content: center;">
                    <form action="{{ route('provider.refunds.response', $refund->id) }}" method="POST" style="display:inline;">
                        @csrf
                        <input type="hidden" name="response" value="accepted">
                        <button type="submit" class="btn btn-success w-100" onclick="return confirm('Terima refund dan teruskan ke admin?')">
                    ...
                    <form action="{{ route('provider.refunds.response', $refund->id) }}" method="POST" style="display:inline;">
                        ...
                        <button type="submit" class="btn btn-danger w-100" onclick="return confirm('Tolak refund?')">
